...
def create_dict(ihec, epi):
    ...
    '''This function receives two files with the md5sum from Ihec and EpiAtlas samples and returns 
    a dictionary where the keys are the md5sums from EpiAtlas and the values are the md5sum from Ihec'''
...
def cell_type_dset(ihec_j, epi_j, dict_ihec_epi):
    ...
    '''This function receives two jsons (from ihec and epiatlas) and the dictionary with the 
    md5sum information (generated by the function above). It will return a list with all matched 
    datasets including the cell_type information'''
